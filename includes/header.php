...
<head>
    ...
    <style>
        ...
        /* ── TASK 8: Skeleton Screen Utilities ─────────────────────────────── */
        @keyframes shimmer {
        ...
        .skeleton-title { height: 28px; width: 60%; margin-bottom: 4px; }
        .skeleton-sub { height: 10px; width: 80%; }
        .skeleton-icon { width: 48px; height: 48px; border-radius: 10px; flex-shrink: 0; }
        .skeleton-card { opacity: 1; transition: opacity 0.3s; }
        .skeleton-card.loaded { opacity: 0; pointer-events: none; position: absolute; }
    ...
    <div class="app-layout">
        ...
        <aside id="sidebar" class="sidebar flex flex-col">
            ...
            <div class="px-3 py-2 border-b border-gray-100">
                <div class="relative">
                    <input type="text" id="sidebarSearch" placeholder="ค้นหาเมนู..." autocomplete="off"
                        class="w-full pl-8 pr-3 py-1.5 text-xs bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:border-green-400 focus:ring-1 focus:ring-green-400/30 transition">
                    <i class="fas fa-search absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 text-xs"></i>
                    <button id="sidebarSearchClear" class="hidden absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 text-xs" onclick="clearSidebarSearch()">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
            </div>
            ...
            <!-- Navigation -->
            <nav class="flex-1 overflow-y-auto py-2" id="sidebarNav">
                <!-- Quick Access Section -->
            ...
            <div class="border-t border-gray-100 p-3">
                <div class="flex items-center gap-2 p-2 rounded-xl hover:bg-gray-50 transition cursor-default">
                    <div class="w-8 h-8 rounded-lg flex items-center justify-center text-white text-sm font-bold flex-shrink-0"
                        style="background: linear-gradient(135deg, #00B900 0%, #047857 100%)">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="text-[10px] text-gray-400 capitalize"><?= htmlspecialchars($currentUser['role'] ?? 'admin') ?></div>
                    </div>
                    <a href="<?= $baseUrl ?>auth/logout.php" title="Logout"
                        class="w-7 h-7 flex items-center justify-center rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 transition flex-shrink-0">
                        <i class="fas fa-sign-out-alt text-xs"></i>
                    </a>
                </div>
                <div class="text-center text-[9px] text-gray-300 mt-1">LINE CRM Pro v3.5</div>
            </div>
        </aside>
        ...
        <div class="main-content">
            ...
            <header class="top-header">
                <div class="flex items-center min-w-0">
                    <button onclick="toggleSidebar()" class="md:hidden mr-3 text-gray-500 hover:text-gray-700 flex-shrink-0">
                        <i class="fas fa-bars text-lg"></i>
                    </button>
                    <!-- Breadcrumb Navigation -->
                    <nav class="flex items-center min-w-0" aria-label="Breadcrumb">
                        <?php
                        // Build breadcrumb from $breadcrumbs if set, else fallback to page title
                        $bc = $breadcrumbs ?? [['label' => $pageTitle ?? 'Dashboard', 'url' => null]];
                        $bcCount = count($bc);
                        foreach ($bc as $i => $crumb):
                            $isLast = ($i === $bcCount - 1);
                        ?>
                            <?php if ($i > 0): ?>
                                <i class="fas fa-chevron-right text-gray-300 text-xs mx-1.5 flex-shrink-0"></i>
                            <?php endif; ?>
                            <?php if (!$isLast && !empty($crumb['url'])): ?>
                                <a href="<?= htmlspecialchars($crumb['url']) ?>"
                                   class="text-sm text-gray-400 hover:text-green-600 transition truncate flex-shrink-0 hidden sm:block">
                                    <?= htmlspecialchars($crumb['label']) ?>
                                </a>
                            <?php else: ?>
                                <h1 class="page-title truncate"><?= htmlspecialchars($crumb['label']) ?></h1>
                            <?php endif; ?>
                        <?php endforeach; ?>
                    </nav>
                </div>
            ...
            <div class="content-area">
                ...
                <script>
                    ...
                    function filterSidebarMenu(q) {
                        ...
                        if (!q) {
                            // Restore all items
                            nav.querySelectorAll('.menu-section, .menu-item, .nested-menu-item, .nested-menu-group, .nested-menu-parent').forEach(el => {
                                el.style.display = '';
                            });
                            return;
                        }
                        ...
                        // Show matching items
                        nav.querySelectorAll('.nested-menu-item, .menu-item').forEach(item => {
                            ...
                            if (matches) {
                                ...
                                const submenu = item.closest('.menu-submenu, .nested-submenu');
                                if (submenu) submenu.classList.add('open');
                                // Show parent group
                                const group = item.closest('.nested-menu-group');
                                if (group) group.style.display = '';
                                const groupParent = group?.querySelector('.nested-menu-parent');
