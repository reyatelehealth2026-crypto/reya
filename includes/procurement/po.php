...
$purchaseOrders = $poService->getAllPOs($filters);
...
<div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    <a href="?tab=po&status=draft" class="bg-white rounded-xl shadow p-4 hover:shadow-lg <?= $statusFilter === 'draft' ? 'ring-2 ring-gray-400' : '' ?>">
    ...
    <a href="?tab=po&status=submitted" class="bg-white rounded-xl shadow p-4 hover:shadow-lg <?= $statusFilter === 'submitted' ? 'ring-2 ring-blue-400' : '' ?>">
        <div class="flex items-center">
    ...
    <a href="?tab=po&status=partial" class="bg-white rounded-xl shadow p-4 hover:shadow-lg <?= $statusFilter === 'partial' ? 'ring-2 ring-yellow-400' : '' ?>">
        <div class="flex items-center">
    ...
    <a href="?tab=po&status=completed" class="bg-white rounded-xl shadow p-4 hover:shadow-lg <?= $statusFilter === 'completed' ? 'ring-2 ring-green-400' : '' ?>">
        <div class="flex items-center">
...
<div class="bg-white rounded-xl shadow">
    <div class="p-4 border-b flex justify-between items-center flex-wrap gap-2">
        ...
        <div class="flex gap-2">
            <?php if ($statusFilter): ?>
            <a href="?tab=po" class="px-3 py-2 bg-gray-200 rounded-lg text-sm">ดูทั้งหมด</a>
            <?php endif; ?>
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y">
                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-4 py-3">
                        <a href="inventory/po-detail.php?id=<?= $po['id'] ?>" class="font-mono text-blue-600 hover:underline"><?= htmlspecialchars($po['po_number']) ?></a>
                    </td>
                    ...
                    <td class="px-4 py-3 text-center">
                        <a href="inventory/po-detail.php?id=<?= $po['id'] ?>" class="p-2 text-blue-600 hover:bg-blue-50 rounded inline-block">
                            <i class="fas fa-eye"></i>
                        </a>
                        <?php if (in_array($po['status'], ['submitted', 'partial'])): ?>
                        <a href="?tab=gr&po_id=<?= $po['id'] ?>" class="p-2 text-green-600 hover:bg-green-50 rounded inline-block" title="รับสินค้า">
...
document.getElementById('createForm').addEventListener('submit', async function(e) {
    ...
    if (result.success) {
        window.location.href = 'inventory/po-detail.php?id=' + result.data.id;
    } else {
